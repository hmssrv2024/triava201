...
const Chatbot: React.FC<ChatbotProps> = ({ isMinimized }) => {
    ...
    const resetInactivityTimer = useCallback(() => {
        ...
        if (messages.length > 1 && !isLoading && !isMinimized && !document.hidden && mode === 'chat') {
             inactivityTimerRef.current = window.setTimeout(sendInactivityPrompt, INACTIVITY_TIMEOUT);
        }
    }, [sendInactivityPrompt, messages.length, isLoading, isMinimized, mode]);
